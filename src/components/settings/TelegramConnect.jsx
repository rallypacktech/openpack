      ...
      if (profiles.length > 0 && onProfileUpdate) {
        ...
        if (profiles[0].telegram_chat_id) {
          ...
          if (typeof pendo !== "undefined") {
            pendo.track("telegram_connected", {
              has_username: !!profiles[0].telegram_username,
            });
          }
          toast.success("Telegram connected successfully!");
        } else {
          setError(
            "No Telegram connection detected yet. Make sure you sent the message in Telegram, then try again. If you're using a VPN, try disabling it — VPNs can block the connection.",
          );
        }
      } else {
        setError(
          "Couldn't find your profile. Try disconnecting and reconnecting.",
        );
      }
    } catch (e) {
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <p className="text-sm text-muted-foreground">
          Get free, best-effort alerts via Telegram when an evacuation is
          ordered or a shelter opens near you. Each alert includes the original
          timestamp and delivery attempt number so you can tell if information
          may be stale.
        </p>
        ...
        {isConnected ? (
          ...
          <div className="space-y-3">
            <div className="p-3 bg-blue-50 rounded-lg space-y-1">
              ...
              <p className="text-xs text-blue-700">
                Tap a button below to open Telegram with a pre-filled message,
                then press send.
              </p>
            </div>
            ...
            <div className="p-3 bg-muted/50 rounded-lg space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                After sending the message in Telegram, come back and tap below.
              </p>
              <Button
                variant="outline"
            ...
            {showManual && (
              <div className="p-3 bg-amber-50 rounded-lg space-y-2 text-xs text-amber-900">
                <p className="font-medium">
                  Manual setup (for DuckDuckGo / privacy browsers):
                </p>
                <p>
                  1. Open Telegram and search for{" "}
                  <strong>@{connectInfo.botUsername}</strong>
                </p>
          ...
          <div className="space-y-3">
            ...
            <p className="text-xs text-muted-foreground">
              Opens Telegram with a pre-filled message. Send it to link your
              account, then come back and tap below.
            </p>
          </div>
        )}
        ...
        <div className="text-xs text-muted-foreground bg-muted/50 p-3 rounded-lg space-y-1">
          <p>
            <strong>Free & best-effort:</strong> Telegram alerts depend on your
            internet connection. During tower outages, messages may be delayed —
            that's why each alert carries its original timestamp. Always verify
            critical status in the RallyPack app.
          </p>
          <p>
            <strong>VPN users:</strong> If the connection doesn't go through,
            try disabling your VPN and tapping "Check connection" again — VPNs
            can block Telegram's webhook delivery.
          </p>
        </div>
      </CardContent>
    </Card>
