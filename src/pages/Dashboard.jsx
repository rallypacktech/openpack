...
export default function Dashboard() {
  ...
  const handleSaveAddress = async () => {
    try {
      ...
      if (
        !locationForm.street_address ||
        !locationForm.city ||
        !locationForm.state_province ||
      ...
      if (typeof pendo !== "undefined") {
        pendo.track("onboarding_address_saved", {
          state_province: cleanedData.state_province || "",
          country: cleanedData.country || "",
          fema_region: cleanedData.fema_region || "",
          has_coordinates: !!(cleanedData.latitude && cleanedData.longitude),
          household_size: cleanedData.household_size || 1,
          has_children: !!cleanedData.has_children,
          has_pets: !!cleanedData.has_pets,
        });
      }
        ...
        .then(() => {
          ...
          if (typeof pendo !== "undefined") {
            pendo.track("onboarding_completed", {
              has_address: !!userProfile.street_address,
    ...
    try {
      ...
      if (profileData.length > 0) {
        ...
        if (profileData[0].latitude && profileData[0].longitude) {
          fetchWeather(
            profileData[0].latitude,
            profileData[0].longitude,
      ...
      Promise.all([
        ...
        base44.entities.ProductRecommendation.filter(
          { active: true },
          "-priority",
          3,
        ),
      ]).then(
        ([cachesResponse, spotsResponse, firstAidData, notifData, allRecs]) => {
          ...
          if (notifData.length === 0) {
            ...
            const relevantRecs = allRecs
              .filter((rec) => {
                if (
                  rec.family_member_types &&
                  rec.family_member_types.length > 0
                ) {
                  return rec.family_member_types.some((type) =>
            ...
            const recNotifications = relevantRecs.map((rec) => ({
              ...
              message:
                rec.description ||
                `Consider adding this to your emergency cache`,
              type: "info",
              read: false,
              recommendation: rec,
            }));
            ...
            const hasEvacuationNotice = notifData.some(
              (notif) =>
    ...
    try {
      ...
      if (data.current) {
        ...
        setWeather({
          ...
          highLow: data.daily
            ? `H: ${convertTemp(data.daily.temperature_2m_max[0])}${tempUnit} / L: ${convertTemp(data.daily.temperature_2m_min[0])}${tempUnit}`
            : null,
          unit: tempUnit,
        });
        ...
        if (code >= 95) {
          generatedAlerts.push({
            ...
            message:
              "Severe thunderstorm detected in your area. Move to safe shelter immediately.",
            severity: "danger",
          });
          setEmergencyMode(true);
        }
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-cream">
        <div
          className="w-6 h-6 border-2 border-foreground/20 border-t-foreground rounded-full animate-spin"
          role="status"
    ...
    return (
      <div className="min-h-screen bg-red-50">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-red-100 mt-1">
                  Critical alerts active in your area
                </p>
              </div>
              <button
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          {alerts.length > 0 && (
            <section
              className="bg-white rounded-lg shadow-lg p-6 mb-6 border-4 border-red-500"
          ...
          {notifications.filter(
            (n) =>
              n.type === "alert" ||
              n.title.toLowerCase().includes("evacuation"),
          ).length > 0 && (
            <section
              className="bg-white rounded-lg shadow-lg p-6 mb-6 border-4 border-orange-500"
              aria-labelledby="evacuation-notices-heading"
            >
              <h2
                id="evacuation-notices-heading"
          ...
          {primaryMeetSpot && (
            <section
              className="bg-white rounded-lg shadow-lg p-6 mb-6 border-2 border-blue-500"
              aria-labelledby="primary-meetspot-heading"
            >
              <h2
                id="primary-meetspot-heading"
          ...
          >
            ...
            <button
              onClick={() =>
                navigate(createPageUrl("Resources") + "?tab=meetspots")
              }
            ...
            <button
              onClick={() =>
                navigate(createPageUrl("Resources") + "?tab=firstaid")
              }
          ...
          {weather && (
            <section
              className="bg-white rounded-lg shadow-lg p-6 border-2 border-gray-300"
  ...
  const withinWeek =
    agreedDate &&
    Date.now() - new Date(agreedDate).getTime() < 7 * 24 * 60 * 60 * 1000;
  const needsTermsAgreement =
    dataLoaded && !sessionTermsAcked && !sameVersion && !withinWeek;
      ...
      <TermsAgreement
        onAgree={async (version, date) => {
          try {
            ...
            const previousVersion = userProfile?.terms_agreed_version || "";
            if (userProfile) {
            ...
            if (typeof pendo !== "undefined") {
              pendo.track("terms_agreement_accepted", {
  ...
  if (isOnboarding) {
    const currentStep = needsAddress
      ? 1
      : needsFamilySetup
        ? 2
        : needsMeetSpots
    ...
    return (
      <div className="min-h-screen bg-cream font-sans">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8 space-y-5">
          ...
          >
            <h2 id="onboarding-progress" className="sr-only">
              Onboarding Progress
            </h2>
            <div className="flex items-center justify-between mb-4" role="list">
              {["Address", "Family", "Meet Spots", "Caches"].map((label, i) => {
                ...
                return (
                  <div
                    key={label}
                    className={`flex items-center gap-1.5 text-xs font-sans ${active ? "text-foreground font-semibold" : done ? "text-primary" : "text-muted-foreground"}`}
                    role="listitem"
                  >
                    <div
                      className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-semibold ${active ? "bg-foreground text-background" : done ? "bg-primary text-white" : "bg-secondary text-muted-foreground"}`}
                    >
                      {done ? "✓" : stepNum}
                    </div>
            ...
            <article className="bg-white border border-border rounded p-7">
              <div className="text-center mb-6">
                ...
                <h2 className="font-serif text-2xl font-semibold text-foreground mb-2">
                  Step 1: Add Your Home Address
                </h2>
                <p className="text-muted-foreground font-sans text-sm max-w-md mx-auto">
                  Your address enables localized weather alerts, tailored
                  disaster recommendations, and nearby emergency resources.
                </p>
              </div>
          ...
          {needsFamilySetup && !needsAddress && (
            <article className="bg-white border border-border rounded p-7">
              <div className="text-center mb-6">
                ...
                <p className="text-muted-foreground font-sans text-sm max-w-md mx-auto">
                  Build your household roster so everyone gets the right
                  supplies and stays connected.
                </p>
              </div>
              <div className="space-y-5">
                <div className="bg-secondary/50 p-4 rounded">
                  ...
                  <ul className="text-sm text-gray-700 space-y-1">
                    ...
                    <li>
                      ...
                      Supplies tailored to your household's needs
                    </li>
                    <li>
                      • <strong>Track medical requirements:</strong> Keep
                      everyone's health info organized
                    </li>
                  </ul>
                </div>
                ...
                <FamilyMemberForm
                    ...
                    try {
                      ...
                      for (const member of data.members) {
                        const cleanMember = {
                          ...
                          medical_conditions:
                            member.medical_conditions || undefined,
                          emergency_contact:
                            member.emergency_contact || undefined,
                      ...
                      for (const pet of data.pets) {
                        const cleanPet = {
                          ...
                          medical_conditions:
                            pet.medical_conditions || undefined,
                      ...
                      if (typeof pendo !== "undefined") {
                        const petSpecies = data.pets
                          .map((p) => p.species)
                          .filter(Boolean);
                        pendo.track("onboarding_family_saved", {
                          member_count: data.members.length,
                          pet_count: data.pets.length,
                          pet_species: petSpecies.join(", "),
                          was_skipped: false,
                        });
                      }
                      ...
                      console.error(
                        "Error saving family members and pets:",
                        error,
                      );
                      alert(
                  ...
                  onSkip={async (data) => {
                    try {
                      ...
                      if (
                        data &&
                        (data.members.length > 0 || data.pets.length > 0)
                      ) {
                      ...
                      if (typeof pendo !== "undefined") {
                        const memberCount = data ? data.members.length : 0;
                        const petCount = data ? data.pets.length : 0;
                        const petSpecies = data
                          ? data.pets.map((p) => p.species).filter(Boolean)
                          : [];
                        pendo.track("onboarding_family_saved", {
                ...
                <p className="text-xs text-muted-foreground text-center font-sans">
                  Update family anytime in Settings
                </p>
              </div>
            </article>
          )}
          ...
          {needsMeetSpots && !needsAddress && !needsFamilySetup && (
            <article className="bg-white border border-border rounded p-7">
              <div className="text-center mb-6">
                ...
                <h2 className="font-serif text-2xl font-semibold text-foreground mb-2">
                  Step 3: Define Meeting Spots
                </h2>
                <p className="text-muted-foreground font-sans text-sm max-w-md mx-auto">
                  When cell service fails, where will your family meet?
                </p>
              </div>
              <div className="space-y-4">
                <div className="bg-secondary/50 p-4 rounded">
                  ...
                  <ul className="text-sm text-gray-700 space-y-1">
                    ...
                    <li>
                      • Family members may be separated at work, school, or on
                      the road
                    </li>
                    <li>
                      • Having predetermined spots in all directions prevents
                      confusion
                    </li>
                    <li>
                      • FEMA recommends multiple meetup locations for different
                      scenarios
                    </li>
                  </ul>
                </div>
                <div className="bg-primary/5 border-l-2 border-primary p-4 rounded-r">
                  <p className="text-sm font-sans text-foreground">
                    <strong>Real scenario:</strong> During Hurricane Harvey,
                    families couldn't contact each other for days. Those with
                    pre-planned meeting spots reunited faster.
                  </p>
                </div>
                <button
                  onClick={() =>
                ...
                <p className="text-xs text-muted-foreground text-center font-sans">
                  Manage in Resources → Meet Spots
                </p>
              </div>
            </article>
          )}
            ...
            !needsMeetSpots && (
              <article className="bg-white border border-border rounded p-7">
                <div className="text-center mb-6">
                  ...
                  <h2 className="font-serif text-2xl font-semibold text-foreground mb-2">
                    Step 4: Build Emergency Caches
                  </h2>
                  <p className="text-muted-foreground font-sans text-sm max-w-md mx-auto">
                    Be prepared with the supplies that matter before you need
                    them.
                  </p>
                </div>
                <div className="space-y-4">
                  <div className="bg-secondary/50 p-4 rounded">
                    ...
                    <ul className="text-sm text-gray-700 space-y-1">
                      ...
                      <li>
                        • Families without proper documentation get redirected
                        or denied
                      </li>
                      <li>
                        • Having organized caches means you grab and go—no
                        scrambling
                      </li>
                      <li>
                        • Track expiration dates so supplies are always ready
                      </li>
                    </ul>
                  </div>
                  <div className="bg-primary/5 border-l-2 border-primary p-4 rounded-r">
                    <p className="text-sm font-sans text-foreground">
                      <strong>Common mistake:</strong> Arriving at shelters
                      without pet supplies, prescriptions, or ID. These cause
                      families to be turned away at critical moments.
                    </p>
                  </div>
                  <button
                    onClick={() => navigate(createPageUrl("Resources"))}
    ...
    <div className="min-h-screen bg-cream font-sans">
      ...
      <div className="bg-white border-b border-border">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-5">
          <p className="text-xs uppercase tracking-widest font-sans text-muted-foreground mb-0.5">
            Command Center
          </p>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <StatsCard
                ...
                subtitle={
                  caches.length > 0
                    ? `${caches[0]?.name || "Emergency Kit"} - Updated recently`
                    : "No caches yet"
                }
                icon={Package}
                ...
                completionPercent={
                  caches.length > 0
                    ? Math.round(((caches[0].items?.length || 0) / 20) * 100)
                    : 0
                }
              />
              <StatsCard
                ...
                subtitle={
                  primaryMeetSpot
                    ? `Primary: ${primaryMeetSpot.name}`
                    : meetSpots.length > 0
                      ? "Set up directional coverage"
                      : "No meet spots set"
                }
                icon={MapPin}
                onView={() =>
