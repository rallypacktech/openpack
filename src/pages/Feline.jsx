...
const CHECKLIST = [
  ...
  { icon: Heart, title: "72-Hour Food, Water & Litter", desc: "Pre-bag food, pack a small collapsible litter box, and include a familiar toy or blanket to reduce stress." },
  { icon: FileText, title: "Vaccination Proof & Microchip", desc: "Shelters and boarding facilities require rabies records. A microchip dramatically improves reunification odds." },
  { icon: MapPin, title: "Pet-Friendly Lodging Identified", desc: "Cats cannot go to public emergency shelters. Book your backup pet-friendly hotel before the storm season." },
  { icon: AlertTriangle, title: "Hiding Behavior Plan", desc: "Cats hide when scared. Confine your cat to one room before you start packing so they don't disappear." },
  { icon: Package, title: "Medications & Health Records", desc: "Insulin, thyroid meds, or allergy treatments can't wait. Pack a 7-day supply and keep your vet's number offline." },
];
...
export default function Feline() {
  ...
  const heroUrl = useHeroShot("feline");
    ...
    <div className="min-h-screen bg-[#F5F0E8] font-sans text-[#1C1C1A]">
      ...
      <section className="relative h-[80vh] min-h-[560px] flex items-end">
        ...
        <div className="relative w-full max-w-7xl mx-auto px-6 pb-20 md:pb-28">
          ...
          <p className="font-sans text-base md:text-lg text-white/75 max-w-md mb-10 leading-relaxed">
            A cat that disappears under a bed when chaos starts can cost you everything. RallyPack helps cat owners build a plan that accounts for feline behavior — before it's too late to think clearly.
          </p>
      ...
      <section className="bg-[#1C1C1A] text-white py-20">
        <div className="max-w-6xl mx-auto px-6 grid grid-cols-1 md:grid-cols-3 gap-0 divide-y md:divide-y-0 md:divide-x divide-white/10">
          {[
            { n: "46M+", unit: "", label: "US households own a cat — yet cat-specific emergency planning resources are nearly nonexistent" },
            { n: "10–30", unit: "min", label: "The time a stressed cat can disappear in your home before an evacuation — making them impossible to find" },
            { n: "2%", unit: "", label: "Of lost cats in shelters are reunited with their owners — vs. 26% of dogs. Microchipping closes that gap." },
          ].map((s) => (
      ...
      <section className="relative h-[50vh] min-h-[340px] overflow-hidden">
        <div className="absolute inset-0 bg-cover bg-[center_40%]" style={{ backgroundImage: "url('https://images.unsplash.com/photo-1573865526739-10659fec78a5?w=1600&q=80')" }} />
      ...
      <section className="relative overflow-hidden bg-[#1C1C1A]">
        ...
        <div className="relative max-w-5xl mx-auto px-6 py-32 text-center">
          ...
          <h2 className="font-serif text-4xl sm:text-5xl md:text-6xl font-bold text-white leading-none mb-8">
            What's your plan if your cat<br />
            <em className="not-italic text-[#D64A2E]">hides when the alarm goes off?</em>
          </h2>
          <p className="text-base font-sans text-white/55 max-w-md mx-auto mb-12 leading-relaxed">
            Take the free readiness quiz or create your RallyPack account to build a complete plan for your cat.
          </p>
